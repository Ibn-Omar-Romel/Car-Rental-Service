<?php
    ...
    if( $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ( !empty($name)) {
            ...
            $stmt = $conn->prepare("UPDATE Driver_Informations SET Driver_Name = ? WHERE Driver_ID = ?");
            $stmt->bind_param("si", $name, $_SESSION['Driver_ID']);
            $stmt->execute();

            if ($stmt->affected_rows > 0) {
                echo "";
            } else {
                $message = "No rows updated. Driver_ID may not exist or the value is already the same.";
            }
        }


        // Updating Driver Email
        if (preg_match('/[A-Z\s]/', $email)) {
            $message = "Email contains Uppercase Letter or Space.";
        }
        else if (!empty($email)) {

            $stmt = $conn->prepare("UPDATE Driver_Informations SET Driver_Email = ? WHERE Driver_ID = ?");
            $stmt->bind_param("si", $email, $_SESSION['Driver_ID']);
            $stmt->execute();
        ...
        // Updating Driver Contact Number
        if (!preg_match('/^(013|014|015|016|017|018|019)\d{8}$/', $num)) {
            $message = 'Invalid Contact Number';
        }
        else if (!empty($num)) {

            $stmt = $conn->prepare("UPDATE Driver_Informations SET Driver_Contact_Number = ? WHERE Driver_ID = ?");
            $stmt->bind_param("si", $num, $_SESSION['Driver_ID']);
            $stmt->execute();
        ...
        // Updating Driver Emergency Contact Number
        if (!preg_match('/^(013|014|015|016|017|018|019)\d{8}$/', $emer_num)) {
            $message = 'Invalid Contact Number';
        }
        else if (!empty($emer_num)) {

            $stmt = $conn->prepare("UPDATE Driver_Informations SET Driver_Emergency_Number = ? WHERE Driver_ID = ?");
            $stmt->bind_param("si", $emer_num, $_SESSION['Driver_ID']);
            $stmt->execute();
        ...
        if ( !empty($gender)) {
            ...
            $stmt = $conn->prepare("UPDATE Driver_Informations SET Driver_Gender = ? WHERE Driver_ID = ?");
            $stmt->bind_param("si", $gender, $_SESSION['Driver_ID']);
            $stmt->execute();
        ...
        if ( !empty($addr)) {
            ...
            $stmt = $conn->prepare("UPDATE Driver_Informations SET Driver_Address = ? WHERE Driver_ID = ?");
            $stmt->bind_param("si", $addr, $_SESSION['Driver_ID']);
            $stmt->execute();
